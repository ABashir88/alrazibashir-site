          ...
          {experience.map((role, i) => (
            <article key={role.company} className="card-hover" style={{
              ...
              <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "space-between", alignItems: "flex-start", gap: "0.5rem" }}>
                <div>
                  {/* ── HIGH PRIORITY FIX 2: Current role badge on Telnyx ── */}
                  <div style={{ display: "flex", alignItems: "center", gap: "0.6rem", marginBottom: "0.3rem" }}>
                    <h3 style={{ fontSize: i === 0 ? "1.1rem" : "1rem", fontWeight: 700, color: "var(--color-text)", margin: 0, fontFamily: "var(--font-body)" }}>
                      {role.title}
                    </h3>
                    {i === 0 && (
                      <span style={{
                  ...
                  <p style={{
                    fontSize: i === 0 ? "0.9rem" : "0.85rem",
                    color: i === 0 ? "var(--color-accent)" : "var(--color-text-secondary)",
                    margin: "0 0 0.2rem", fontFamily: "var(--font-body)", fontWeight: 500
                  }}>
                    {role.company}
                  </p>
                </div>
